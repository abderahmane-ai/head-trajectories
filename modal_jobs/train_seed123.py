"""
modal_jobs/train_seed123.py — Modal training job for seed 123 (replication run 1).

Identical architecture and hyperparameters to seed 42.
Uses a separate checkpoint Volume for clean isolation.
Reuses the shared probe dataset Volume constructed by seed 42.
...
APP_NAME          = "trajectories-seed123"
VOLUME_NAME       = "trajectories-ckpts-seed123"
PROBE_VOLUME_NAME = "trajectories-probe"   # shared with seed42
...
def train() -> None:
    ...
    print(f"\n{'=' * 64}")
    print(f"  Developmental Trajectories — Seed 123 Training Run")
    print(f"  Device : {device} ({torch.cuda.get_device_name(0)})")
    print(f"{'=' * 64}\n")

    # ── Probe dataset — load only (constructed by seed 42 run) ───────────────
    probe_path = PROBE_DIR / "probe_dataset.pt"
    print("  [Probe] Loading shared probe dataset...")
...
def main() -> None:
    print("Submitting seed-123 training job to Modal...")
    train.remote()
    print("Job submitted.")
