...
\textcolor{Muted}{\sffamily Author} & Abderahmane Ai &
...
Thresholds are validated before normalization. Non-finite thresholds are
rejected. If a threshold is non-positive, it is preserved in recorded raw
diagnostics but replaced by a small positive floor \emph{only for division}:
\[
\tau_m^{\mathrm{eff}} =
\begin{cases}
\tau_m, & \tau_m > 0, \\
\varepsilon, & \tau_m \le 0,
\end{cases}
\qquad \varepsilon = 10^{-6}.
\]

This is a defensive numerical fallback, not the intended scientific state.

\subsection{Decision rule}

Define normalized scores
\[
z_m = \frac{s_m}{\tau_m^{\mathrm{eff}}}.
\]
The label set is
\[
\mathcal{Y}
=
\{\texttt{UNDIFFERENTIATED},\texttt{SINK},\texttt{PREV\_TOKEN},
...
\end{cases}
\]
where $z_{(1)} \ge z_{(2)} \ge \cdots$ are the sorted normalized scores and
\[
\delta = 0.05
\]
is the tie tolerance.
