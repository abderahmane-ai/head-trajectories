...
each head is scored on five behaviors, an empirical null is calibrated from a
...
\item an empirical null is calibrated relative to an architecture-matched random
baseline (with threshold summaries retained only as diagnostics);
...
\item dominant-summary labels compress potentially mixed behaviors into one
reported type unless the active-set view is inspected;
...
computed for every head, an empirical null is calibrated with a causally valid
random baseline, and head identities are tracked as trajectories. The result is a
...
significance with a causally valid random baseline (empirical null + FDR), and
analyzing both activation and dominance trajectories over training.
\end{abstractbox}
